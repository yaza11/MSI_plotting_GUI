    ...
    with open(re.findall(r'(?:file:///)?(.+)', file_path)[0]) as f:
        ...
        for i, line in enumerate(f):
            ...
            if i != 0:
                ...
                spectra_mzs.append(mzs)
                spectra_intensities.append(intensities)
                spectra_SNR.append(signal_to_noise_ratios)
    return pixel_names, spectra_mzs, spectra_intensities, spectra_SNR


def create_FT_for_comps(
        mz: str | float,
        pixel_names: list[str],
        spectra_mzs: list[list[float]],
    ...
    **kwargs : TYPE
    ...
    for idx_pixel, (pixel, mzs, data_point) in enumerate(zip(
        ...
        if norm_spectra and (len(data_point) > 0):
            m = np.median(data_point)
            data_point = np.divide(
                data_point,
                m,
                out=np.zeros_like(data_point),
                where=data_point != 0
            )
        img_x[idx_pixel] = int(re.findall('X(.*)Y', pixel)[0])  # x coordinate
        img_y[idx_pixel] = int(re.findall('Y(.*)', pixel)[0])  # y coordinate
        x = np.array(mzs, dtype=float)  # mz values of spectrum
        if len(data_point) == 0:
            array[idx_pixel] = 0
            continue
        elif kernel_mode == 'gauss':
            # normal distribution around mz
            window_fct = normal_distribution(x, mz, width_filter)
        elif kernel_mode == 'rect':
            window_fct = rect_function(x, mz, width_filter)
        elif kernel_mode == 'max':
            window_fct = rect_function(x, mz, width_filter) * width_filter
            array[idx_pixel] = np.max(
                np.array(data_point, dtype=float) * window_fct)
        # intensities around mz_ weighted by normal distribution
        array[idx_pixel] = np.sum(
            data_point * window_fct)
...
class Spectra:
    pixel_names: list[str]
    spectra_mzs: list[float]
    spectra_intensities: list[float]
    spectra_SNR: list[float]
...
class UI(QtWidgets.QMainWindow):
    ...
    def link_widgets(self):
        self.btn_read = self.findChild(QtWidgets.QPushButton, 'btn_read_data')
        self.btn_read.clicked.connect(self.read_data)

        self.btn_read_masses = self.findChild(QtWidgets.QPushButton, 'btn_read_masses')
        self.btn_read_masses.clicked.connect(self.read_mass_file)

        self.btn_browse_data = self.findChild(QtWidgets.QPushButton, 'btn_browse_file_spectra')
        self.btn_browse_data.clicked.connect(self.get_txt_file_from_dialog)

        self.btn_browse_masses = self.findChild(QtWidgets.QPushButton, 'btn_browse_masses')
        self.btn_browse_masses.clicked.connect(self.get_mass_file_from_dialog)

        self.btn_browse_img_folder = self.findChild(QtWidgets.QPushButton, 'btn_browse_img_folder')
        self.btn_browse_img_folder.clicked.connect(self.get_img_dir_from_dialog)

        self.btn_plot = self.findChild(QtWidgets.QPushButton, 'btn_plot')
        self.btn_plot.clicked.connect(self.plt_comp)

        self.btn_save_plot = self.findChild(QtWidgets.QPushButton, 'btn_save_plot')
    ...
    def read_mass_file(self):
        ...
        if mass_file.is_okay:
            ...
            # hook up prev next
            self.btn_next = self.findChild(QtWidgets.QPushButton, 'btn_next_mass')
            self.btn_prev = self.findChild(QtWidgets.QPushButton, 'btn_prev_mass')
            self.btn_all_mass = self.findChild(QtWidgets.QPushButton, 'btn_all_mass')
    ...
    def plt_all(self):
        ...
        while self.mass_file.idx < np.unique(self.mass_file.get_indizes()).max() and (not self.stop):
    ...
    def create_FT_for_comps(self, mz: str | float) -> pd.DataFrame:
        mz = float(mz)
        # if mode is gauss, width defines sigma
        # otherwise widh is the width of the window
        # default values for the two modes
        mode_to_width = {'gauss': .001,
                         'rect': .003,
                         'max': .003}
        if self.opts.width_filter is None:
            width_filter = mode_to_width[self.opts.mode]
        N_pixels = len(self.spectra.pixel_names)
        ...
        # iterate over pixels (=lines in txt file)
        for idx_pixel, (pixel, mzs, data_point) in enumerate(zip(
                self.spectra.pixel_names, self.spectra.spectra_mzs, self.spectra.get_data(self.opts.plt_SNR)
        )):
            img_x[idx_pixel] = int(re.findall('X(.*)Y', pixel)[0])  # x coordinate
            img_y[idx_pixel] = int(re.findall('Y(.*)', pixel)[0])  # y coordinate
            x = np.array(mzs, dtype=float)  # mz values of spectrum
            if self.opts.kernel_mode == 'gauss':
                # normal distribution around mz
                window_fct = normal_distribution(x, mz, self.opts.width_filter)
            elif self.opts.kernel_mode == 'rect':
                window_fct = rect_function(x, mz, self.opts.width_filter)
            elif self.opts.kernel_mode == 'max':
                window_fct = rect_function(x, mz, self.opts.width_filter) * self.opts.width_filter
                array[idx_pixel] = np.max(
                    np.array(data_point, dtype=float) * window_fct)
            # intensities around mz_ weighted by normal distribution
            array[idx_pixel] = np.sum(
